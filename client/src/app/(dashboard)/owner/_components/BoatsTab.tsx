...
import {
    fetchAllBoatsAdmin, deleteBoat, updateBoat, createBoatAdmin,
    fetchAllUsers, AdminBoat, AdminUser
...
export function BoatsTab() {
    ...
    const load = useCallback(async () => {
        try {
            setLoading(true);
            const [b, a] = await Promise.all([fetchAllBoatsAdmin(), fetchAllUsers('agent')]);
            setBoats(b);
            setAgents(a);
        } catch (err: unknown) {
    ...
    const handleAdd = async () => {
        ...
        setSaving(true);
        try {
            await createBoatAdmin({ name: newName.trim(), agentId: newAgentId || null });
            setNewName(''); setNewAgentId(''); setShowAdd(false);
            await load();
        } catch (err: unknown) {
            const e = err as { response?: { data?: { message?: string } } };
            toast(e.response?.data?.message || 'Failed to add boat', 'error');
    ...
    const handleSaveEdit = async (id: number) => {
        ...
        try {
            ...
            const e = err as { response?: { data?: { message?: string } } };
            toast(e.response?.data?.message || 'Failed to update boat', 'error');
        } finally { setSaving(false); }
    };
    ...
    const handleDelete = async (id: number, name: string) => {
        ...
        try {
            ...
            const e = err as { response?: { data?: { message?: string } } };
            toast(e.response?.data?.message || 'Failed to delete boat', 'error');
        }
    };
        ...
        <div>
            <div className="flex items-center justify-between mb-10">
                <div>
                    ...
                    <p className="text-ocean-600 text-[10px] font-black uppercase tracking-[0.2em] mt-1">{boats.length} registered boats active</p>
                </div>
                        ...
                        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }}
                            ...
                            <div className="space-y-6 relative z-10">
                                <div>
                                 ...
                                 <div>
                                    <label className="block text-[10px] font-black text-ocean-600 uppercase tracking-widest mb-3 ml-1">Assign {t.agent}</label>
                                    <select
                                        value={newAgentId} onChange={e => setNewAgentId(e.target.value ? Number(e.target.value) : '')}
                                        className="w-full bg-gray-50 border border-ocean-100 rounded-2xl px-5 py-4 text-ocean-950 font-bold focus:outline-none focus:border-ocean-500 transition-all appearance-none cursor-pointer"
                                    >
                                        ...
                                        <td className="px-8 py-5">
                                            {editingId === boat.id ? (
                                                ...
                                                <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border ${
                                                    boat.status === 'active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100 shadow-sm shadow-emerald-500/10' : 'bg-gray-50 text-ocean-200 border-ocean-50'
                                                }`}>
                                                    {boat.status === 'active' ? t.active : t.inactive}
                                                </span>
                                            )}
                                        </td>
                                            ...
                                            <div className="flex items-center justify-end gap-3 translate-x-4 group-hover:translate-x-0 transition-transform">
                                                {editingId === boat.id ? (
                                                    ...
                                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button onClick={() => { setEditingId(boat.id); setEditName(boat.name); setEditAgentId(boat.agent_id ?? ''); setEditStatus(boat.status || 'active'); }}
