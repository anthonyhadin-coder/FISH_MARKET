...
export const getAllUsers = catchAsync(async (req: AuthRequest, res: Response) => {
    ...
    const [users] = await pool.query(`
      SELECT DISTINCT u.id, u.name, u.phone, u.role, u.is_active, u.created_at
      FROM users u
      WHERE u.id = ?
      OR u.id IN (
        SELECT DISTINCT agent_id FROM boats
        WHERE owner_id = ? AND agent_id IS NOT NULL
      )
    `, [ownerId, ownerId]);
    res.json(users);
});
...
export const deleteBoat = catchAsync(async (req: AuthRequest, res: Response) => {
    ...
    const ownerId = req.user?.userId;
    const [result]: any = await pool.query(
        'DELETE FROM boats WHERE id = ? AND owner_id = ?',
        [id, ownerId]
    );
...
export const getBoatYearlyReport = catchAsync(async (req: AuthRequest, res: Response) => {
    ...
    const [rows]: any = await pool.query(`
        ...
        FROM sales s
        LEFT JOIN (
          SELECT MONTH(date) as m, SUM(amount) as expenses
          FROM expenses WHERE boat_id = ? AND YEAR(date) = ?
          GROUP BY MONTH(date)
        ) e ON e.m = MONTH(s.date)
        LEFT JOIN (
          SELECT MONTH(date) as m, SUM(amount) as payments
          FROM boat_payments WHERE boat_id = ? AND YEAR(date) = ?
          GROUP BY MONTH(date)
        ) p ON p.m = MONTH(s.date)
        WHERE s.boat_id = ? AND YEAR(s.date) = ?
