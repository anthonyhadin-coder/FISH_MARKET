...
export const getDailyReport = catchAsync(async (req: AuthRequest, res: Response) => {
    ...
    // Base matching logic for the current user and optional date/boat filtering
    let matchAgentOrOwnerObj = '';
    const matchParams: any[] = [];
    
    if (role === 'agent') {
        matchAgentOrOwnerObj = 'agent_id = ?';
        matchParams.push(agentId);
    } else if (role === 'owner') {
         // For simplicity, we filter by owner later via join or assume boatId is passed
         // If needed later we can join boats to filter by owner
         matchAgentOrOwnerObj = '1=1'; // placeholder 
    } else {
         matchAgentOrOwnerObj = '1=1';
    }
...
export const getOwnerBoatWeekly = catchAsync(async (req: AuthRequest, res: Response) => {
    ...
    const [rule]: any = await pool.query(`
        ...
        WHERE agent_id = (SELECT agent_id FROM sales WHERE boat_id = ? LIMIT 1)
