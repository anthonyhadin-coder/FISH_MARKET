...
// ─────────────────────────────────────────────────────────────────
// 11. UNICODE TAMIL NUMBER PARSER (unchanged behaviour)
// ─────────────────────────────────────────────────────────────────

describe('Tamil Unicode Number Parser', () => {
...
// ─────────────────────────────────────────────────────────────────
// 12. ROMANISED TAMIL NUMBER PARSER — unit tests
// ─────────────────────────────────────────────────────────────────

describe('Romanised Tamil Number Parser — parseRomanisedTamilNumber()', () => {

  // ── Base numbers ─────────────────────────────────────────────────
  it('["aayiram"] → 1000, unambiguous', () => {
    const r = parseRomanisedTamilNumber(['aayiram']);
    expect(r?.value).toBe(1000);
    expect(r?.isAmbiguous).toBe(false);
    expect(r?.tokensConsumed).toBe(1);
  });

  it('["nooru"] → 100, unambiguous', () => {
    const r = parseRomanisedTamilNumber(['nooru']);
    expect(r?.value).toBe(100);
    expect(r?.isAmbiguous).toBe(false);
  });

  it('["pathu"] → 10, unambiguous', () => {
    const r = parseRomanisedTamilNumber(['pathu']);
    expect(r?.value).toBe(10);
    expect(r?.isAmbiguous).toBe(false);
  });

  // ── Multiplier × Base compounds ──────────────────────────────────
  it('["rendu", "aayiram"] → 2000, unambiguous', () => {
    const r = parseRomanisedTamilNumber(['rendu', 'aayiram']);
    expect(r?.value).toBe(2000);
    expect(r?.isAmbiguous).toBe(false);
    expect(r?.tokensConsumed).toBe(2);
  });

  it('["moonu", "aayiram"] → 3000, unambiguous', () => {
    const r = parseRomanisedTamilNumber(['moonu', 'aayiram']);
    expect(r?.value).toBe(3000);
    expect(r?.isAmbiguous).toBe(false);
  });

  it('["naalu", "nooru"] → 400, unambiguous', () => {
    const r = parseRomanisedTamilNumber(['naalu', 'nooru']);
    expect(r?.value).toBe(400);
    expect(r?.isAmbiguous).toBe(false);
  });

  // ── Additive sequences (compound + trailing English digit) ───────
  it('["aayiram", "500"] → 1500, ambiguous', () => {
    const r = parseRomanisedTamilNumber(['aayiram', '500']);
    expect(r?.value).toBe(1500);
    expect(r?.isAmbiguous).toBe(true);
    expect(r?.tokensConsumed).toBe(2);
  });

  it('["rendu", "aayiram", "500"] → 2500, ambiguous', () => {
    const r = parseRomanisedTamilNumber(['rendu', 'aayiram', '500']);
    expect(r?.value).toBe(2500);
    expect(r?.isAmbiguous).toBe(true);
    expect(r?.tokensConsumed).toBe(3);
  });

  it('["nooru", "50"] → 150, ambiguous', () => {
    const r = parseRomanisedTamilNumber(['nooru', '50']);
    expect(r?.value).toBe(150);
    expect(r?.isAmbiguous).toBe(true);
  });

  // ── Additive guard: trailing integer ≥ compound → NOT additive ───
  it('["aayiram", "2000"] → 1000 only (2000 ≥ 1000, not additive)', () => {
    const r = parseRomanisedTamilNumber(['aayiram', '2000']);
    expect(r?.value).toBe(1000);
    expect(r?.tokensConsumed).toBe(1);
  });

  // ── Fused compound words ──────────────────────────────────────────
  it('["pathinaayiram"] → 10000, unambiguous', () => {
    const r = parseRomanisedTamilNumber(['pathinaayiram']);
    expect(r?.value).toBe(10000);
    expect(r?.isAmbiguous).toBe(false);
    expect(r?.tokensConsumed).toBe(1);
  });

  it('["ainnooru"] → 500, unambiguous', () => {
    const r = parseRomanisedTamilNumber(['ainnooru']);
    expect(r?.value).toBe(500);
    expect(r?.isAmbiguous).toBe(false);
  });

  // ── startAt offset ────────────────────────────────────────────────
  it('startAt skips anchor tokens — ["rate", "aayiram"] startAt=1 → 1000', () => {
    const r = parseRomanisedTamilNumber(['rate', 'aayiram'], 1);
    expect(r?.value).toBe(1000);
    expect(r?.tokensConsumed).toBe(1);
  });

  // ── Non-Tamil tokens return null ──────────────────────────────────
  it('["vanjaram", "50"] → null (no Tamil numeral)', () => {
    const r = parseRomanisedTamilNumber(['vanjaram', '50']);
    expect(r).toBeNull();
  });
});

// ─────────────────────────────────────────────────────────────────
// 13. VOICE PARSER INTEGRATION — Tamil rate extraction
// ─────────────────────────────────────────────────────────────────

describe('Voice Parser — Tamil rate extraction', () => {

  it('"vanjaram 20 kilo rate aayiram" → rate = 1000', () => {
    const s = saleOf('vanjaram 20 kilo rate aayiram');
    expect(s?.rate).toBe(1000);
    expect(s?.rateIsAmbiguous).toBeFalsy();
  });

  it('"vanjaram 20 kilo rate aayiram 500" → rate = 1500, ambiguous', () => {
    const s = saleOf('vanjaram 20 kilo rate aayiram 500');
    expect(s?.rate).toBe(1500);
    expect(s?.rateIsAmbiguous).toBe(true);
    expect(s?.warnings?.some(w => w.includes('confirm'))).toBe(true);
  });

  it('"sankara 30 kilo rate rendu aayiram" → rate = 2000', () => {
    const s = saleOf('sankara 30 kilo rate rendu aayiram');
    expect(s?.rate).toBe(2000);
    expect(s?.rateIsAmbiguous).toBeFalsy();
  });

  it('"sankara 30 kilo rate rendu aayiram 500" → rate = 2500, ambiguous', () => {
    const s = saleOf('sankara 30 kilo rate rendu aayiram 500');
    expect(s?.rate).toBe(2500);
    expect(s?.rateIsAmbiguous).toBe(true);
  });

  it('"nethili 15 kilo rate nooru 50" → rate = 150', () => {
    const s = saleOf('nethili 15 kilo rate nooru 50');
    expect(s?.rate).toBe(150);
    expect(s?.rateIsAmbiguous).toBe(true);
  });

  it('"kaala 25 kilo rate nooru" → rate = 100, unambiguous', () => {
  });
});
