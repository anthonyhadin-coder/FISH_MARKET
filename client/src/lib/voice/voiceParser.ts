import { parseNumber } from './tamilNumberParser';
...
function extractRate(text: string, priceRange?: [number, number]): number | null {
  // With explicit keyword (rate, price, at, @, ரேட், விலை, ₹)
  const kw = text.match(/(?:rate|ரேட்|விலை|price|at|@)\s*(\d+)/i);
  if (kw) {
    const v = parseFloat(kw[1]);
    if (v >= 10 && v <= 9999) return v;
  }
  // Currency pattern
  const cur = text.match(/(\d+)\s*(?:rupee|ரூபாய்|rs|₹)/i);
  if (cur) return parseFloat(cur[1]);

  // Largest standalone number — apply fish price range sanity check
  // Only use fallback if there's no weight keyword that could claim the number
  ...
  const numbers = [...text.matchAll(/\b(\d+)\b/g)]
    ...
    .filter(n => {
      if (priceRange) {
        ...
        // Allow 50% outside range to accommodate deals
        return n >= min * 0.5 && n <= max * 2;
      }
      return n >= 10 && n <= 9999;
    });

  // If there's a weight unit in the text, skip the fallback number extraction
  // (the number is the weight, not the rate)
  if (hasWeightUnit) return null;
...
export const parseVoiceInput = (
  ...
  for (let si = 0; si < segments.length; si++) {
    ...
    const rate   = extractRate(seg, profile?.priceRange);
