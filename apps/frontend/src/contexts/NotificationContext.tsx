...
import { AppNotification } from '@fishmarket/shared-types';
...
export const NotificationProvider = ({ children, agentData }: NotificationProviderProps) => {
    ...
    const pendingCount = agentData?.pendingCount ?? 0;
    const isSyncing = agentData?.isSyncing ?? false;
    const dailyReport = agentData?.dailyReport;
    const selectedBoat = agentData?.selectedBoat;
